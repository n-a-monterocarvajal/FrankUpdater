...
| F-53 | Integración con Aurora Store | Flujo | P2 | Propuesta del usuario: usar un Aurora Store instalado en vez de suplantarlo. Aurora (80f9d81) solo expone enlaces de entrada: `market://details?id=`, `https://play.google.com/store/apps/details?id=` (tras su pantalla de confirmación), `SHOW_APP_INFO` y `SEND` de texto; no ofrece ningún servicio ni proveedor para buscar o descargar en segundo plano desde otra app. | Si Aurora está instalado, un botón "Abrir en Aurora" en Buscar y en las tarjetas que abre la ficha de la app en Aurora; si no, una nota de que con Aurora instalado existe esa vía. Operar en segundo plano requeriría una API que Aurora no tiene (se podría proponer a sus autores). **Descartado (19 de septiembre):** un traspaso deja fuera todo lo que FrankUpdater aporta (varias fuentes, elección de la última versión compatible, verificación y actualización desatendida); solo ahorraría abrir Aurora a mano. |
