...
| F-44 (Buscar) | La pantalla Buscar se divide en cuatro bloques: "Buscar apps" (un campo y su acción), "Opciones avanzadas" plegado (página de APKMirror, historial de APKPure, versiones preliminares), "Versiones encontradas" fuera de la tarjeta de controles y la tarjeta de descarga. Con un nombre escrito, el botón principal es buscar en la web; "Consultar fuentes" solo aparece con un paquete válido. | API 36: búsqueda por nombre, elección de app y listado de versiones correctos. API 23: los bloques caben con fuente grande y el plegado funciona. |
| F-60 | La ficha de Play ofrece "Comprobar compatibilidad en todas las fuentes", que pasa el paquete a la consulta multifuente de arriba, la misma que usan los resultados de actualización. | API 23, con acceso anónimo: Signal (`org.thoughtcrime.securesms`, no instalada) pasa de "Pendiente de comprobar" a "Última versión conocida: 175401 · Última compatible: 175001". |
...
| F-60 | Buscar → Google Play | Flujo | P1 | Al llegar a una app por la búsqueda de Play, "Última compatible según metadatos" queda en "Pendiente de comprobar" y no hay forma de lanzar la comprobación desde ahí: había que volver arriba y escribir el paquete a mano. Es el caso de quien busca una app que no tiene instalada. | Un botón en la ficha que pase ese paquete a la consulta multifuente. |
