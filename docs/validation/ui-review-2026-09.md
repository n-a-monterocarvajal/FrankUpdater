...
| F-27 (parcial) | Al volver del ajuste "Instalar apps desconocidas" con el permiso concedido, la instalación se reanuda sola; si no se concedió, el mensaje lo dice. La explicación previa sigue siendo el mensaje "Autoriza a FrankUpdater para instalar paquetes". | Compilado y verificado con `verify.ps1`. Falta probar el retorno: exige revocar el permiso, que es un ajuste de seguridad. |
...
- F-27: la reanudación está implementada; probarla exige revocar y volver a conceder el permiso de instalación, que es un ajuste de seguridad.
